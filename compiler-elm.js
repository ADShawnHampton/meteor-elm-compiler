...
Plugin.registerCompiler({
    ...
    var compiler = new ElmCompiler();
    return compiler;
});

function ElmCompiler () {}
...
ElmCompiler.prototype.processFilesForTarget = function (files) {
    ...
    for (var i = 0; i < elmMainFiles.length; i++) {
        ...
        if (elmMainFile != null) {
            compileElm(elmMainFile, elmOutputDir)
        }
    }
};
...
function compileElm (elmMainFile, elmOutputDir) {
    var currentDir = process.env.PWD;
